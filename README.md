...
-- | -- | --
image_size | 150 | Resizes   images to a standard size, balancing computational cost and feature   retention.
batch_size | 64 | Optimizes   memory usage and computational efficiency during training.
learning_rate | 0.0005 | Provides a   fine-tuned balance for gradient updates, ensuring stable convergence.
dropout_rate | 0.3 | Reduces   overfitting by randomly disabling 30% of neurons during training.
optimizer | Adam | Adaptive   learning rate optimization enhances training speed and efficiency.
Scheduler = gamma | 0.5 | Halves the   learning rate every 5 epochs to improve learning during later stages.
num_epochs | 20 | Enables   sufficient iterations for the model to converge.
patience | 5 | Implements   early stopping to prevent unnecessary training epochs if validation loss does   not improve.
...
Augmentation techniques are applied to enhance model generalization.

Dataset split into training (80%), validation (10%), and testing (10%) sets.

2. CNN Architecture:

Four convolutional layers progressively extract features from input images.

Max-pooling layers reduce spatial dimensions, ensuring efficient computation.

Fully connected layers classify the extracted features into the defined labels.

Dropout layers mitigate overfitting.

3. Model Training:

The model uses CrossEntropyLoss to calculate classification loss.

Metrics such as accuracy, precision, recall, and F1-score are tracked.

4. Evaluation:

Training and validation losses are plotted.

Final metrics are computed using the test set.

5. Prediction:

A function predicts the type of tumor based on a provided MRI image.
